...
backbone_changes='mod'

if os.path.exists(r'F:\论文工作\鲁棒PV预测\RLMC-main\RLMC-main\sensitivity_env\\'+backbone_changes):
    DATA_DIR=r'sensitivity_env\\'+backbone_changes
else:
    DATA_DIR = 'mod_dataset'
print('use dataset as',DATA_DIR)
...
class DDPGAgent:
    ...
    def select_action(self, obs):
        # flops,params=profile(self.actor,obs)
    ...
    def update(self,
        ...
        # flops,params=profile(self.critic,(batch_obs, sampled_actions))
        # flops, params = clever_format([flops, params], '%.3f')
        # print('critic')
        # print(f"运算量：{flops}, 参数量：{params}")
...
def pretrain_actor(obs_dim, act_dim, hidden_dim, states, train_error, cls_weights, 
    ...
    for epoch in trange(200, desc='[Pretrain]'):
        ...
        pretrain_losses.append(np.average(epoch_loss))
    ...
    pretrain_losses=pd.DataFrame(pretrain_losses)
    pretrain_losses.to_csv(r'F:\论文工作\鲁棒PV预测\RLMC-main\RLMC-main\convergence\\pretrain_'+str(int(time.time()))+'.csv')
    return best_actor
...
def run_rlmc(use_weight=True, use_td=True, use_extra=True, use_pretrain=True, epsilon=0.3):
    ...
    valid_preds = np.load(f'{DATA_DIR}/bm_valid_preds.npy')[:train_X.shape[0],]
    test_preds = np.load(f'{DATA_DIR}/bm_test_preds.npy')[:train_X.shape[0],]
    ...
    def get_batch_rewards(env, idxes, actions):
        ...
        for i in range(len(idxes)):
            ...
            mape_reward = get_mae_reward(q_mae, new_mae, 1)
            # mae_reward  = get_mae_reward(q_mae, new_mae, 2)
            combined_reward = mape_reward + rank_reward
            mae_lst.append(new_mae)
            rewards.append(combined_reward) # ------------- here is a revision 2024/12/12 20:32
    ...
    if use_pretrain:
        ...
        for param, target_param in zip(
            ...
            target_param.data.copy_(param.data)
    toc = time.time()
    warm_time = (toc - tic) /train_error.shape[0]
    ...
    for epoch in trange(512):
        ...
        step_losses.append(np.array([valid_mae_loss,valid_mape_loss,np.average(q_loss_lst),np.average(target_q_lst)]))
    ...
    step_losses=pd.DataFrame(step_losses)
    step_losses.columns=['MAE','MAPE','q_loss','target_q_loss']
    if use_pretrain:
        step_losses.to_csv(r'F:\论文工作\鲁棒PV预测\RLMC-main\RLMC-main\convergence\\convergence_pre_True'+str(int(time.time()))+'.csv')
    else:
        step_losses.to_csv(
            r'F:\论文工作\鲁棒PV预测\RLMC-main\RLMC-main\convergence\\convergence_pre_Flase' + str(int(time.time())) + '.csv')
    for param, target_param in zip(agent.actor.parameters(), best_actor.parameters()):
        param.data.copy_(target_param)
    toc=time.time()
    train_time=(toc-tic)/train_error.shape[0]

    tic=time.time()
    test_mae_loss, test_mape_loss, count_lst = evaluate_agent(
        agent, test_states, test_preds, test_y)
    toc=time.time()
    test_time=(toc-tic)/test_states.shape[0]
    print(f'test_mae_loss: {test_mae_loss:.3f}\t'
          f'test_mape_loss: {test_mape_loss*100:.3f}')

    print('pretrain_time',warm_time)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_pretrain', action='store_false', default=False)
